...
void	create_ray(t_scene *scene, t_ray *ray, double x, double y)
{
	...
	p = v_sum(v_sum(scene->screen.p1, v_scalar_mul(scene->cam->dx, v_norm(v_sub(scene->screen.p1, scene->screen.p2)) * x / scene->w)), v_scalar_mul(scene->cam->up, -1. * v_norm(v_sub(scene->screen.p1, scene->screen.p4)) * y / scene->h));
