...
void	cylinder_axis_mod(void *param, t_v axis, int key)
{
    ...
    if(key == S_KEY || key == PG_DOWN_KEY || key == A_KEY || \
        ...
        cylinder->c = v_sum(cylinder->c, v_scalar_mul(axis, MOVE_EPSILON));
    else if (key == ARROW_DX_KEY || key == ARROW_DOWN_KEY || \
                key == ARROW_UP_KEY || key == ARROW_SX_KEY)
    {
    	cylinder->n = v_normalize(rotate_vector(cylinder->n, axis, ROT_EPSILON));
    	cylinder->up = v_normalize(rotate_vector(cylinder->up, axis, ROT_EPSILON));
	    cylinder->dx = v_normalize(rotate_vector(cylinder->dx, axis, ROT_EPSILON));
    }
    set_cylinder_orientation(cylinder);
}
